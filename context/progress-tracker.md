...
- A test-only `EditorLayout` component and a `/editor` demo route composing the navbar + sidebar were built and verified, then removed (not part of the defined scope). No route currently mounts `editor-navbar.tsx` / `project-sidebar.tsx`; that composition belongs to whichever feature unit defines the real editor page layout.
